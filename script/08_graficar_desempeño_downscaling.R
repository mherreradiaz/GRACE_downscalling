...
data_metrics <- map_dfr(resultados, 'metrics') |> 
  ...
  mutate(product == ifelse(product == 'GLDAS_GWS','GLDAS_GWSA',product),
         rmse = ifelse(product =='GLDAS_GWSA',rmse/10,rmse),
         mae = ifelse(product =='GLDAS_GWSA',mae/10,mae))
data_folds <- map_dfr(resultados, 'folds') |> 
  select(fecha,everything()) |> 
  mutate(product == ifelse(product == 'GLDAS_GWS','GLDAS_GWSA',product),,
         mean = ifelse(product =='GLDAS_GWSA' & (.metric == 'rmse' | .metric == 'mae'),mean/10,mean))
...
data_metrics |>
  ...
  complete.dates(fecha, product, metrica) |> 
  ggplot(aes(x = as.Date(fecha), y = valor)) +
  geom_hline(yintercept = 0,linetype = 'dotted',col = 'black',alpha = .7) +
  ...
  facet_grid2(rows = vars(metrica), cols = vars(product),
              scales = 'free') +
  labs(x = NULL, y = NULL) +
  scale_x_date(date_labels = "%Y", breaks = seq(as.Date('2003-01-01'), as.Date('2023-01-01'), by = '4 years'),
               minor_breaks = seq(as.Date('2003-01-01'), as.Date('2023-01-01'), by = '1 year'),
               limits = c(as.Date('2002-11-01'),as.Date('2024-01-01')),expand = c(.04, 0)) +
  scale_y_continuous(expand = c(.12,0)) +
  theme_bw() +
  theme(strip.background = element_rect(fill = 'white'),
        panel.spacing = unit(0, 'lines'))

ggsave('output/fig/downscaling/model_metrics.png',width = 8, height = 5,scale = 1)
...
data_metrics |>
  ...
  mutate(metrica = factor(metrica, levels = c('RSQ', 'RMSE', 'MAE'))) |> 
  complete.dates(fecha, product, metrica) |> 
  ggplot(aes(fecha,valor, color = product)) +
  geom_hline(yintercept = 0,linetype = 'dotted',col = 'black',alpha = .7) +
  ...
  scale_x_date(date_labels = "%Y", breaks = seq(as.Date('2003-01-01'), as.Date('2023-01-01'), by = '4 years'),
               minor_breaks = seq(as.Date('2003-01-01'), as.Date('2023-01-01'), by = '1 year'),
               limits = c(as.Date('2002-11-01'),as.Date('2024-01-01')),expand = c(.01, 0)) +
...
ggsave('output/fig/downscaling/model_metrics_alt.png',width = 8, height = 6, scale = 1)
...
data_metrics |>
  ...
  facet_wrap(metrica ~ product, scales = "free_y",axes = 'all_y',axis.labels = 'margins') +
  labs(x = NULL, y = NULL) +
...
ggsave('output/fig/downscaling/model_metrics.png',width = 10, height = 6, scale = 1)
...
ggsave('output/fig/downscaling/model_folds_metrics.png',width = 10, height = 6, scale = 1)

# vip

data_vip |>
  group_by(product,fecha) |> 
  slice_max(importance, n = 1) |> 
  group_by(product,variable) |> 
  summarise(freq = n()) |> 
  ggplot(aes(x = reorder(variable, freq), y = freq)) +
  geom_col(width = 1,fill = 'darkcyan',color = 'darkslategrey') +
  coord_flip() +
  scale_y_continuous(expand = c(0,0),limits = c(0,128)) +
  scale_x_discrete(expand = c(0,0)) +
  labs(x = NULL, y = 'count of months as top predictor') +
  facet_wrap(~product, scales = 'free_y', ncol = 1, strip.position = 'left') +
  theme_bw()

ggsave('output/fig/downscaling/model_vip_count.png',width = 10, height = 6, scale = 1)
