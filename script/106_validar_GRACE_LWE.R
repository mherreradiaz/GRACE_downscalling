...
data_GWL <- read_rds('data/processed/rds/GWD_chile.rds')
pozos <- vect('data/processed/vectorial/pozos_chile.gpkg')
  
grace_files <- dir_ls('data/processed/raster/GRACE_highres_old/')
fechas <- str_extract(grace_files,'\\d{4}-\\d{2}')
grace_r <- grace_files |> 
  rast() |> 
  setNames(fechas)
...
data <- data_grace |> 
  ...
  mutate(GWL = as.numeric(scale(GWD))) |> 
  ungroup() |> 
  select(fecha,cuenca,codigo,GWL,lwe)

cor_general <- data |> # correlación general
  reframe(r = cor(GWL, lwe, method = 'pearson', use = 'complete.obs'))

cor_mes_pozo <- data |> # correlacion temporal a nivel de pozo
  group_by(codigo) |> 
  reframe({
    n <- sum(complete.cases(GWL, lwe))
    if (n > 3) {
      test <- cor.test(GWL, lwe, method = 'pearson', use = 'complete.obs')
...
left_join(pozos, y = cor_mes_pozo) |> 
  writeVector('data/processed/vectorial/cor_GRACE.gpkg',
              overwrite=T)
...
data_raw <- read_rds('data/processed/rds/cor_GRACE_mes_pozo.rds')
...
ggsave('output/fig/validation/GRACE/r_histogram.png',width = 8, height = 6)
...
ggsave('output/fig/validation/GRACE/n_histogram.png',width = 8, height = 6)
