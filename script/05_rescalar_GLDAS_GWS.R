...
gldas_files <- dir_ls('data/processed/raster/GLDAS_monthly/GWS/')
fechas_gldas <- str_extract(gldas_files,'\\d{4}-\\d{2}')
...
min_date <- max(c(min_date_preds,min(fechas_gldas)))
max_date <- min(c(max_date_preds,max(fechas_gldas)))

fechas_modelo <- fechas_gldas[between(fechas_gldas,min_date,max_date)]
...
grid_nonNA <- mask(grid, gldas_r[[1]])
...
resultados_pts <- map(sample_evenly(1:nlyr(gws),4), \(i) {
  ...
  if (nrow(data_modelo) < 10 || length(unique(data_modelo$GWS)) <= 1) {
    return(met = tibble(
  ...
  model_rec <- recipe(GWS ~ ., data = data_train) |>
...
resultados_pts_10 <- map(sample_evenly(1:nlyr(gws),4), \(i) {
...
resultados_pts_20 <- map(sample_evenly(1:nlyr(gws),4), \(i) {
...
resultados_pts_30 <- map(sample_evenly(1:nlyr(gws),4), \(i) {
...
resultados_pts_40 <- map(sample_evenly(1:nlyr(gws),4), \(i) {
...
resultados <- map(seq_along(fechas_modelo), \(i) {
  ...
  # Cargar variable objetivo (GWS) como raster independiente
  gws_layer <- setNames(gldas_r[[i]], 'GWS')
  
  # Crear stack de predictores (excluyendo GWS)
  out_preds <- c(
    rast(lapply(r_preds, \(pred) pred[[i]])),
    setNames(DEM, 'DEM')
  )
  
  # Extraer GWS a puntos
  gws_vals <- terra::extract(gws_layer, vect(sample_pts), cores = 10) |> 
    select(-ID)
  ...
  data_modelo <- bind_cols(gws_vals, preds_vals) |> 
    drop_na()
  ...
  gws_preds <- predict(model_last_fit |> extract_workflow(), as.data.frame(out_preds))
  
  # Crear raster de predicciones con la geometría del primer predictor
  out <- out_preds[[1]]
  values(out) <- gws_preds$.pred
  out <- mask(out, chl)
  
  # Exportar raster de predicciones
  writeRaster(out, glue('data/processed/raster/GLDAS_GWS_highres/GLDAS_GWS_highres_{fechas_modelo[i]}.tif'), 
              overwrite = TRUE)
...
write_rds(ds_metrics,'data/processed/rds/GLDAS_GWS_downscaling_metrics.rds')
write_rds(ds_folds,'data/processed/rds/GLDAS_GWS_downscaling_folds.rds')
write_rds(ds_vip,'data/processed/rds/GLDAS_GWS_downscaling_VIP.rds')
write_rds(ds_stats,'data/processed/rds/GLDAS_GWS_downscaling_preds_stats.rds')
...
data_metrics <- read_rds('data/processed/rds/GLDAS_GWS_downscaling_metrics.rds')
data_folds <- read_rds('data/processed/rds/GLDAS_GWS_downscaling_folds.rds')
data_vip <- read_rds('data/processed/rds/GLDAS_GWS_downscaling_VIP.rds')
data_stats <- read_rds('data/processed/rds/GLDAS_GWS_downscaling_preds_stats.rds')
...
ggsave('output/fig/downscaling/GLDAS_GWS/model_metrics.png',width = 10, height = 6)
...
ggsave('output/fig/downscaling/GLDAS_GWS/model_folds_metrics.png',width = 10, height = 6)
...
ggsave('output/fig/downscaling/GLDAS_GWS/model_vip_count.png',width = 10, height = 6)
...
ggsave('output/fig/downscaling/GLDAS_GWS/model_vip_month.png',width = 12, height = 5)
...
ggsave('output/fig/downscaling/GLDAS_GWS/model_stats_mean.png',width = 13, height = 7)
...
ggsave('output/fig/downscaling/GLDAS_GWS/model_stats_sd.png',width = 13, height = 7)
